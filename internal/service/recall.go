...
var (
	ErrStateFormatError      = errors.New("state参数格式错误，应为Uid_Platform_UserTag[_IdempotencyKey]格式")
...
func (s *RecallService) ProcessRecallWithParams(state string) (*RecallParams, []string, error) {
	// state 格式：Uid_Platform_UserTag[_IdempotencyKey]
	// 不需要 URL 解码，直接解析
...
func (s *RecallService) SaveRecall(params *RecallParams, extraParams map[string]string) (*RecallResponse, error) {
	...
	allParams["idempotency_key"] = params.IdempotencyKey
